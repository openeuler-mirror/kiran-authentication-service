...
{
    QDBusInterface *iface = new QDBusInterface(KIRAN_DBUS_INTERFACE, KIRAN_DBUS_PATH, KIRAN_DBUS_INTERFACE, QDBusConnection::systemBus());
    if (!iface->isValid())
    {
        return tr("Failed to request authorization code, please check if the face service (com.kiran.face.service) is running");
    }

    // 使用默认参数：1天
    QJsonObject jsonObj;
    jsonObj.insert("business_id", KIRAN_BUSINESS_ID);
    jsonObj.insert("user_name", QString());  // C1: CLI 模式传空 user_name，后端自动匹配当前登录用户
    jsonObj.insert("expires_in", 24);
    QJsonDocument jsonDoc(jsonObj);
    QString jsonStr = jsonDoc.toJson();

    KLOG_INFO() << "Kiran CLI DBus call CodeGen with args:" << jsonStr;
    QDBusReply<QString> reply = iface->call("CodeGen", jsonStr);

    QString errorMsg;
    if (reply.isValid())
    {
        KLOG_INFO() << "Kiran CLI CodeGen reply:" << reply.value();
        ...
        {
            errorMsg = tr("Failed to request authorization code: %1").arg(getKiranErrorMsg(error_code));
        }
    }
    else
    {
...
{
    QDBusInterface *iface = new QDBusInterface(KIRAN_DBUS_INTERFACE, KIRAN_DBUS_PATH, KIRAN_DBUS_INTERFACE, QDBusConnection::systemBus(), this);
    if (!iface->isValid())
    {
        showResultPage(tr("Failed to request authorization code, please check if the face service (com.kiran.face.service) is running"));
        return;
    }
    ...
    QJsonObject jsonObj;
    jsonObj.insert("business_id", KIRAN_BUSINESS_ID);
    jsonObj.insert("user_name", QString());  // C1: GUI 模式传空，后端自动匹配
    jsonObj.insert("expires_in", expires_in);
    QJsonDocument jsonDoc(jsonObj);
    QString jsonStr = jsonDoc.toJson();
    qInfo() << "Kiran DBus call CodeGen with args:" << jsonStr;
    QDBusReply<QString> reply = iface->call("CodeGen", jsonStr);
    if (reply.isValid())
    {
        KLOG_INFO() << "Kiran CodeGen reply:" << reply.value();
        jsonDoc = QJsonDocument::fromJson(reply.value().toLatin1());
        jsonObj = jsonDoc.object();
        int error_code = jsonObj.value("code").toInt();
        if (error_code != KIRAN_SUCCESS)
        {
            showResultPage(tr("Failed to request authorization code: %1").arg(getKiranErrorMsg(error_code)));
        }
        else
        {
