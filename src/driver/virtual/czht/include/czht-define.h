...
| 4 | 服务器返回错误 | 服务器返回错误信息（具体见error_msg） |
| 5 | 离线模式不允许执行 | 离线模式不允许执行授权码操作 |
| 6 | 记录登录日志失败 | 记录登录日志失败 |
| 7 | 正在执行其他任务 | 同一 `business_id` 已在执行任务 |
| 8 | 任务未执行 | 尝试停止一个不存在的任务 |
| 9 | 无法连接 USB 摄像头 | 摄像头设备未连接或无法打开 |
| 11 | 启动监控进程失败 | 无法启动czht-sign进程（离开检测） |
| 12 | 未搜索到匹配的人员（离线模式） | 本地数据库中未找到匹配的人脸 |
| 13 | 用户已过期 | 绑定的用户账号已过期（不在有效时间范围内） |
| 14 | 用户不在缓存中 | 用户不在缓存中，无法开始离开检测 |
| 15 | 工作模式不支持 | 当前工作模式不支持授权码操作 |
| 101 | 服务器内部错误 | 服务器内部错误，如算法服务异常 |
| 102 | 业务ID不存在 | 业务ID不存在 |
| 104 | 未检测到人脸 | 未检测到人脸 |
| 105 | 未搜索到匹配的人员 | 未搜索到匹配的人员 |
| 106 | 搜索到的人员已被删除 | 搜索到的人员已被删除 |
| 107 | 授权码不存在 | 授权码不存在 |
| 108 | 授权码已过期 | 授权码已过期 |
| 109 | 人脸与授权码绑定的人员不匹配 | 人脸与授权码绑定的人员不匹配 |
| 110 | 授权验证user_id不存在 | 授权验证user_id不存在 |
...
enum CZHT_ERROR_NUM
{
    ...
    CZHT_ERROR_SERVER_RETURN_ERROR = 4,
    CZHT_ERROR_OFFLINE_MODE_NOT_ALLOWED = 5,
    CZHT_ERROR_REPORT_LOGIN_LOG_FAILED = 6,
    ...
    CZHT_ERROR_WORK_MODE_NOT_SUPPORTED = 15,
    CZHT_ERROR_SERVER_INTERNAL_ERROR = 101,
    CZHT_ERROR_BUSINESS_ID_NOT_FOUND = 102,
    CZHT_ERROR_FACE_NOT_DETECTED = 104,
    ...
    CZHT_ERROR_AUTHORIZATION_VERIFICATION_USER_ID_NOT_FOUND = 110,
...
static const QString getCZHTErrorMsg(int errorNum)
{
    ...
    static const QMap<int, QString> CZHT_ERROR_MSG = {
        ...
        {CZHT_ERROR_SERVER_RETURN_ERROR, QObject::tr("server return error")},
        {CZHT_ERROR_OFFLINE_MODE_NOT_ALLOWED, QObject::tr("offline mode not allowed")},
        {CZHT_ERROR_REPORT_LOGIN_LOG_FAILED, QObject::tr("report login log failed")},
        ...
        {CZHT_ERROR_START_MONITOR_PROCESS_FAILED, QObject::tr("start monitor process failed (leave detection)")},
        {CZHT_ERROR_MATCH_PERSON_NOT_FOUND_OFFLINE, QObject::tr("match person not found (offline mode)")},
        {CZHT_ERROR_USER_EXPIRED, QObject::tr("user expired (not within valid time range)")},
        {CZHT_ERROR_USER_NOT_IN_CACHE, QObject::tr("user not in cache, cannot start leave detection")},
        {CZHT_ERROR_WORK_MODE_NOT_SUPPORTED, QObject::tr("work mode not supported")},
        {CZHT_ERROR_SERVER_INTERNAL_ERROR, QObject::tr("server internal error, such as algorithm service exception")},
        {CZHT_ERROR_BUSINESS_ID_NOT_FOUND, QObject::tr("business ID not found")},
        {CZHT_ERROR_FACE_NOT_DETECTED, QObject::tr("face not detected")},
        ...
        {CZHT_ERROR_AUTHORIZATION_VERIFICATION_USER_ID_NOT_FOUND, QObject::tr("authorization verification user_id not found")},
